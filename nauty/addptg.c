...
gotone(graph *g, int m, int n)
{
    ...
    if (digraph)
        writed6(outfile, pg, m, n);
    else if (outcode == GRAPH6)
        writeg6(outfile, pg, m, n);
    else
        writes6(outfile, pg, m, n);
...
int main(int argc, char *argv[])
{
    ...
    for (j = 1; !badargs && j < argc; ++j)
    {
        ...
        if (arg[0] == '-' && arg[1] != '\0')
        {
            ...
            while (*arg != '\0')
            {
                sw = *arg++;
                SWBOOLEAN('l', dolabel)
                else SWBOOLEAN('q', quiet) else SWBOOLEAN('c', cswitch) else SWBOOLEAN('k', kswitch) else SWBOOLEAN('i', in) else SWBOOLEAN('o', out) else SWINT('n', nswitch, extras, ">E addptg -n") else SWRANGE('j', ":-", jswitch, joinmin, joinmax, ">E addptg -j") else SWRANGE('e', ":-", eswitch, emin, emax, ">E addptg -e") else badargs = TRUE;
            }
        }
        else
        {
    ...
    if (extras < 0)
        gt_abort(">E addptg: negative argument for -n\n");
    ...
    if (!quiet)
    {
        ...
        if (dolabel)
            fprintf(stderr, " -l");
        if (nswitch)
            fprintf(stderr, " -n%d", extras);
        if (jswitch)
            fprintf(stderr, " -j%ld:%ld", joinmin, joinmax);
        if (eswitch)
            fprintf(stderr, " -e%ld:%ld", emin, emax);
    ...
    if (!quiet)
        fprintf(stderr,
                ">Z  " COUNTER_FMT " graphs read from %s, " COUNTER_FMT " written to %s; %3.2f sec.\n",
