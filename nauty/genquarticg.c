                                ...
                                if ((j1 != edge1[e2].first) && (j1 != edge1[e2].sec))
                                {
                                    ...
                                    if (mult > multm)
                                    {
                                        if (edge1[e1].cond && edge1[e2].cond)
                                            ...
                                            for (i = 1; i <= n / 3 && rely < 1 && !got_one; i++)
                                            {
                                                if (firsttimey[i])
                                                {
                                                    yi[i] = (g[n - i] | bit[n - i]);
...
int main(int argc, char *argv[])
{
    ...
    for (j = 1; !badargs && j < argc; ++j)
    {
        ...
        if (arg[0] == '-' && arg[1] != '\0')
        {
            ...
            while (*arg != '\0')
            {
                ...
                else SWBOOLEAN('l', canonise) else SWBOOLEAN('c', connec1) else SWBOOLEAN('C', connec2) else SWBOOLEAN('g', graph6) else SWBOOLEAN('s', sparse6) else SWBOOLEAN('h', header) else SWBOOLEAN('q', quiet)
    ...
    /* else if (argnum == 5 || argnum > 6)
         badargs = TRUE;*/
    // argnum will never exceeds 1 here andeven in genbg it never exceeds 2!
