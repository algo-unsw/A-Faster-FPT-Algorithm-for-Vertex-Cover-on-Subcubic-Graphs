...
static TLS_ATTR set dnwork[2 * 1000 * MAXM];
#endif
            ...
            int *count, set *active, int *code, int m, int n)
{
    ...
    split1 = -1;
    hint = 0;
    while (*numcells < n && ((split1 = hint, ISELEMENT(active, split1)) || (split1 = nextelement(active, M, split1)) >= 0 || (split1 = nextelement(active, M, -1)) >= 0))
    {
             ...
             int *count, set *active, int *code, int m, int n)
{
    ...
    hint = 0;
    while (*numcells < n && ((split1 = hint, ISELEMENT1(active, split1)) || (split1 = nextelement(active, 1, split1)) >= 0 || (split1 = nextelement(active, 1, -1)) >= 0))
    {
...
int targetcell(graph *g, int *lab, int *ptn, int level,
                ...
                optionblk *options, statsblk *stats, int m, int n, graph *h)
{
    ...
    DYNALLOC1(set, dnwork, dnwork_sz, 2 * 1000 * m, "densenauty malloc");
#endif

    nauty(g, lab, ptn, NULL, orbits, options, stats, dnwork, 2 * 1000 * m, m, n, h);
}
