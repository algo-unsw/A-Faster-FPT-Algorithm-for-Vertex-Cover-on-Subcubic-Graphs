...
int nextelement(const set *set1, int m, int pos)
{
    setword setwd;
    int w;
...
void permset(const set *set1, set *set2, int m, int *perm)
{
    setword setw;
    int pos, b;
    int w;
...
int orbjoin(int *orbits, const int *map, int n)
{
    int i, j1, j2;
...
void writeperm(FILE *f, const int *perm, boolean cartesian, int linelength, int n)
{
...
void fmperm(const int *perm, set *fix, set *mcr, int m, int n)
{
    int i, k, l;
...
void fmptn(const int *lab, const int *ptn, int level, set *fix, set *mcr, int m, int n)
{
    int i, lmin;
           ...
           boolean digraph, int m, int n)
{
    ...
    if (invarproc != NULL && *numcells < n && level >= minlev && level <= maxlev)
    {
...
void maketargetcell(graph *g, int *lab, int *ptn, int level,
...
void shortprune(set *set1, const set *set2, int m)
{
    int i;
...
void breakout(int *lab, int *ptn, int level, int tc, int tv,
              set *active, int m)
{
    int i, prev, next;
